	...
	<div class="m-c">
		<c:if test="${fn:length(requests) gt 0}">
			...
			<ul>
				<c:forEach items="${requests}" var="request">
					<li>
						<a href="/web/stocket/annonser?p_p_id=Retursidan_WAR_tageportlet&p_p_lifecycle=0&p_p_state=normal&p_p_mode=view&p_p_col_id=column-1&p_p_col_count=1&_Retursidan_WAR_tageportlet_page=viewRequest&_Retursidan_WAR_tageportlet_requestId=${request.id}">${request.title}</a>
					</li>
				</c:forEach>
			</ul>
			 --%>
