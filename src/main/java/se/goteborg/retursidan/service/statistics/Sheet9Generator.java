...
public class Sheet9Generator extends BaseSheetGenerator {
	
    public Sheet9Generator(Workbook workbook, Map<String, Object> model) {
		super(workbook, model);
	}

    public void createSheet() {
    	
    	Sheet sheet = workbook.createSheet("Annons. per kat. förv.bolag, to");
        ...
        pageHeaderCell.setCellValue("Antal publicerade annonser per kategori, per förvaltning/bolag, per år och totalt");
        ...
        for (Category category : allCategories) {
        	...
        	for (MonthData year : yearList) {
                for (Unit unit : allUnits) {
            		...
            		int count = statisticsService.getNumberOfAdsForCategoryAndUnitByPeriod(year.getDateSpan(), category, unit);
            		cell.setCellValue(String.valueOf(count));
                	cellNumber++;
                }
                Cell cellTotal = row.createCell(cellNumber);
                cellTotal.setCellStyle(headerStyle);
                int count = statisticsService.getNumberOfAdsForCategoryByPeriod(year.getDateSpan(), category);
    	...
    	for (MonthData year : yearList) {
            for (Unit unit : allUnits) {
            	cell = row.createCell(cellNumber);
            	cell.setCellStyle(headerStyle);
        		int count = statisticsService.getNumberOfAdsForUnitByPeriod(year.getDateSpan(), unit);
        		cell.setCellValue(String.valueOf(count));
            	cellNumber++;
            }
            Cell cellTotal = row.createCell(cellNumber);
            cellTotal.setCellStyle(headerStyle);
            int count = statisticsService.getNumberOfAdsByPeriod(year.getDateSpan());
            cellTotal.setCellValue(String.valueOf(count));            	
        	cellNumber++;
        }        
    }
