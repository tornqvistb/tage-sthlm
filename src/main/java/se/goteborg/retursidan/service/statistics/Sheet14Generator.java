...
public class Sheet14Generator extends BaseSheetGenerator {
	
    public Sheet14Generator(Workbook workbook, Map<String, Object> model) {
		super(workbook, model);
	}

	
    public void createSheet() {
    	Sheet sheet = workbook.createSheet("Annons. per kat,förv,bol,kontakt");
        ...
        for (Unit unit : allUnits) {
    		...
    		List<UserStats> fullUserList = statisticsService.getUserStatsListByUnitAndPeriodForContact(getLifeTimeSpan(), unit);
    		for (UserStats stats : fullUserList) {
            	...
            	for (MonthData month : monthList) {
            		...
            		int count = statisticsService.getNumberOfAdsForContactByPeriod(month.getDateSpan(), stats.getEmail());
            		dataCell.setCellValue(String.valueOf(count));
            		cellNumber++;
            		if (month.spanOver2Years()) {
                		dataCell = row.createCell(cellNumber);
                		dataCell.setCellStyle(headerStyle);
                		count = statisticsService.getNumberOfAdsForContactByPeriod(month.getDateSpanForYear(), stats.getEmail());
                		dataCell.setCellValue(String.valueOf(count));
                		cellNumber++;
                	}
