...
public class Sheet8Generator extends BaseSheetGenerator {
	
    public Sheet8Generator(Workbook workbook, Map<String, Object> model) {
		super(workbook, model);
	}

	
    public void createSheet() {
    	Sheet sheet = workbook.createSheet("Bokningar totalt per kategori");
        ...
        pageHeaderCell.setCellValue("Antal bokade annonser per kategori, per år och totalt");
        ...
        for (Category category : allCategories) {
        	...
        	//if (category.getParent() != null) {
	        	...
	        	for (MonthData month : monthList) {
	        		...
	        		int count = statisticsService.getNumberOfBookedAdsForCategoryByPeriod(month.getDateSpan(), category);
	        		cell.setCellValue(String.valueOf(count));
	        		cellNumber++;
	        		if (month.spanOver2Years()) {
	            		Cell cellSummary = row.createCell(cellNumber);
	            		cellSummary.setCellStyle(headerStyle);
	            		count = statisticsService.getNumberOfBookedAdsForCategoryByPeriod(month.getDateSpanForYear(), category);
